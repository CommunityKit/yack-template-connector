...
//                 } else {
//                     url = `${ROOT_URL}/top${this.getTopChannelFilters(options)}.json?${this.getSolvedFilters(options)}`;
//                     response = await this.setUrlToken(hasUser, url, options.session.user ? options.session.accessToken.token : null);
...
//                     url = `${ROOT_URL}/latest.json?page=${options.nextPageToken}&${this.getSolvedFilters(options)}`;
//                     response = await this.setUrlToken(hasUser, url, options.session.user ? options.session.accessToken.token : null);
...
//                 } else {
//                     url = `${ROOT_URL}/latest.json?${this.getSolvedFilters(options)}`;
//                     response = await this.setUrlToken(hasUser, url, options.session.user ? options.session.accessToken.token : null);
//                     if(catchErrors(response)){
//                         return catchErrors(response);
//                     }else{
...
//             const thread = await populateThread(topic, options, ROOT_URL);
//             thread.metadata = {};
...
//             const thread = await populateThread(firstPostInThread, options, ROOT_URL);
...
//         const userThreadsData = await getUserCreatedContent(this.pluginContext.axios.get, ROOT_URL, userId, options.nextPageToken, options, this.config);
//         for (const threadData of userThreadsData) {
//             // populate thr

        

//             const thread = await populateThread(threadData, options, ROOT_URL);
